...
def train_eval(
    ...
    with tf.compat.v2.summary.record_if(
        ...
        for _actor_input in actor_input.split('__'):
            if _actor_input == 'state':
                ...
                raise NotImplementedError
        actor_input_spec = tensor_spec.TensorSpec(
            (actor_state_size,),
            dtype=tf.float32
        )
        ...
        for _critic_input in critic_input.split('__'):
            if _critic_input == 'state':
                ...
                raise NotImplementedError
        critic_input_spec = tensor_spec.TensorSpec(
            (critic_state_size,),
            dtype=tf.float32
        )
    ...
    ).unbatch().filter(_filter_invalid_transition).batch(batch_size, drop_remainder=True).prefetch(3)
    ...
    with eval_summary_writer.as_default(), tf.compat.v2.summary.record_if(True):
        ...
        if eval_interval:
            eval_images_ph = tf.compat.v1.placeholder(dtype=tf.uint8, shape=[None] * 5)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        if global_step_val == 0:
            ...
            # Run initial collect.
            logging.info('Global step %d: Running initial collect op.', global_step_val)
            ...
            logging.info('Global step %d: Skipping initial collect op.', global_step_val)
        ...
        for iteration in range(global_step_val, initial_model_train_steps + num_iterations):
            ...
            if log_interval and global_step_val % log_interval == 0:
                logging.info('step = %d, loss = %f',global_step_val, total_loss_val.loss)
            ...
            if eval_interval and global_step_val % eval_interval == 0:
                for _eval_metrics, _eval_py_policy, _eval_render_images_summary, _eval_images_summary in (
                        (eval_metrics, eval_py_policy, eval_render_images_summary, eval_images_summary),
                        (eval_greedy_metrics, eval_greedy_py_policy, eval_greedy_render_images_summary,
                         eval_greedy_images_summary)):
                    compute_summaries(
                        _eval_metrics,
                        eval_py_env,
                        _eval_py_policy,
